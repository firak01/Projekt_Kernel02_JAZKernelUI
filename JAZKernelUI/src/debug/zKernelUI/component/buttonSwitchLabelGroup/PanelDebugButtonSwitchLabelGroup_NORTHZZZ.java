...
//			KernelButtonGroupZZZ<String,AbstractButton> groupButton02 = new KernelButtonGroupZZZ<String,AbstractButton>();
			...
			//ListenerComponentSwitchZZZ listenerComponentSwitch = new ListenerComponentSwitchZZZ(objKernel, this, objEventBroker);      //Erzeugt den Event, wenn in auf den Button gedrück wurde. Merke: Wird 2mal das gleiche ausgewaehlt, so wird kein Event gestartet.
			
			//Sicherstellen, dass die Gruppe den im Button-Click geworfenen Event empfaengt!
			//group1.addListener(listenerComponentSwitch);			
			//objEventBroker.addListenerComponentSwitch(listenerComponentSwitch);
			...
			//######## Das UI gestalten. Die Reihenfolge ist wichtig #################
			//++++ Die Buttons
...
class ActionSwitch extends  KernelActionCascadedZZZ{ //KernelUseObjectZZZ implements ActionListener{
	...
	public boolean actionPerformCustom(ActionEvent ae, boolean bQueryResult) throws ExceptionZZZ {
		...
		if(this.hmIndexed.size() == iIndexCurrent) {
			iIndexCurrent=0;
		}else {
			iIndexCurrent++;
		}
		this.setIndexCurrent(iIndexCurrent);
		
		boolean bActiveState = true;
		SwingWorker4ProgramSWITCH worker = new SwingWorker4ProgramSWITCH(objKernel, panelParent, this.hmIndexed, iIndexCurrent, bActiveState, saFlag);
